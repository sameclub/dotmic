...
DotMicAudio::DotMicAudio(uint32_t sample_rate, UAC_Bits_Per_Sample bps, UAC_SPK_Channels spk_channels, UAC_MIC_Channels mic_channels) {
  if (_uac == NULL) {
...
UAC_SPK_Channels DotMicAudio::speakerChannels() {
  return (UAC_SPK_Channels)_spk_channels;
}
UAC_MIC_Channels DotMicAudio::micChannels() {
  return (UAC_MIC_Channels)_mic_channels;
}
UAC_Bits_Per_Sample DotMicAudio::bitsPerSample() {
  return (UAC_Bits_Per_Sample)_bits_per_sample;
}
uint8_t DotMicAudio::bytesPerSample() {
  return _bytes_per_sample;
}
uint32_t DotMicAudio::sampleRate() {
  return _sample_rate;
}

bool DotMicAudio::mute(UAC_Channel channel) {
...
bool DotMicAudio::mute(UAC_Channel channel, bool muted) {
...
int8_t DotMicAudio::volume(UAC_Channel channel) {
  if (_spk_channels == 0 || channel > _spk_channels) {
    log_e("Bad speaker channel %u", channel);
    return INT8_MIN;
  }
...
bool DotMicAudio::volume(UAC_Channel channel, int8_t volume_db) {
#if CFG_TUD_AUDIO_ENABLE_INTERRUPT_EP
  if (_spk_channels == 0 || channel > _spk_channels) {
    log_e("Bad speaker channel %u", channel);
    return false;
...
void DotMicAudio::applyVolume(void *data, uint16_t len) {
  if (_spk_channels == 0) {
    return;
  }
...
void DotMicAudio::onData(arduino_usb_audio_card_data_handler_t callback) {
  _cb = callback;
}
